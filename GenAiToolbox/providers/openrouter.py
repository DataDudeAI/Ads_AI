...
class OpenRouterProvider:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        self.default_model = "nvidia/llama-3.3-nemotron-super-49b-v1:free"
    ...
    def generate_text(self, 
        ...
        try:
            ...
            # Enhanced fallback for junk/empty output
            ai_text = generated_text if isinstance(generated_text, str) else str(generated_text)
            junk_responses = {"<", "<</", "###", "", None}
            if not ai_text.strip() or ai_text.strip() in junk_responses:
                logger.warning(f"OpenRouter returned junk/empty output: '{ai_text}'. Full raw response: {response.text}")
                ai_text = "No response generated. Please try again or use a different model/provider."
